...
CREATE TABLE IF NOT EXISTS public.brand_creator_notes (
...
CREATE TABLE IF NOT EXISTS public.brand_webhooks (
  ...
  secret_key TEXT NOT NULL DEFAULT encode(extensions.gen_random_bytes(32), 'hex'),
...
DROP POLICY IF EXISTS "Brand members can view their brand's notes" ON public.brand_creator_notes;
...
DROP POLICY IF EXISTS "Brand members can create notes" ON public.brand_creator_notes;
...
DROP POLICY IF EXISTS "Brand members can update their brand's notes" ON public.brand_creator_notes;
CREATE POLICY "Brand members can update their brand's notes"
ON public.brand_creator_notes FOR UPDATE
USING (public.is_brand_member(auth.uid(), brand_id));

DROP POLICY IF EXISTS "Brand members can delete their brand's notes" ON public.brand_creator_notes;
...
DROP POLICY IF EXISTS "Brand members can view their brand's webhooks" ON public.brand_webhooks;
CREATE POLICY "Brand members can view their brand's webhooks"
ON public.brand_webhooks FOR SELECT
USING (public.is_brand_member(auth.uid(), brand_id));

DROP POLICY IF EXISTS "Brand admins can create webhooks" ON public.brand_webhooks;
...
DROP POLICY IF EXISTS "Brand admins can update webhooks" ON public.brand_webhooks;
...
DROP POLICY IF EXISTS "Brand admins can delete webhooks" ON public.brand_webhooks;
CREATE POLICY "Brand admins can delete webhooks"
ON public.brand_webhooks FOR DELETE
USING (public.is_brand_admin(auth.uid(), brand_id));

-- RLS policies for webhook_logs
DROP POLICY IF EXISTS "Brand members can view logs for their webhooks" ON public.webhook_logs;
...
DROP TRIGGER IF EXISTS update_brand_creator_notes_updated_at ON public.brand_creator_notes;
...
CREATE INDEX IF NOT EXISTS idx_brand_creator_notes_brand_id ON public.brand_creator_notes(brand_id);
CREATE INDEX IF NOT EXISTS idx_brand_creator_notes_creator_id ON public.brand_creator_notes(creator_id);
CREATE INDEX IF NOT EXISTS idx_brand_webhooks_brand_id ON public.brand_webhooks(brand_id);
