...
import { Search, ChevronDown, ChevronLeft, ChevronRight, CheckCircle2, XCircle, Clock, BadgeCheck, ArrowUpDown, ArrowUp, ArrowDown, Phone, X, Filter, RefreshCw, Download, DollarSign, Ban, UserCheck, Users, Eye } from "lucide-react";
...
type RetentionStatus = "active" | "at-risk" | "dormant" | "churned";

// Helper to compute user retention status based on account age and activity
const getRetentionStatus = (user: User): RetentionStatus => {
  if (!user.created_at) return "churned";
  const now = new Date();
  const createdAt = new Date(user.created_at);
  const daysSinceCreation = Math.floor((now.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24));
  const hasEarnings = (user.wallets?.total_earned || 0) > 0;
  const hasSocialAccounts = (user.social_accounts?.length || 0) > 0;

  // Active: Has social accounts and recent earnings or new user
  if (hasSocialAccounts && (hasEarnings || daysSinceCreation <= 7)) return "active";
  // At Risk: Has social accounts but no earnings after 7 days
  if (hasSocialAccounts && daysSinceCreation <= 30) return "at-risk";
  // Dormant: Old account with social accounts but inactive
  if (hasSocialAccounts && daysSinceCreation <= 90) return "dormant";
  // Churned: No social accounts or very old inactive account
  return "churned";
};
...
const getStatusBadge = (status: RetentionStatus) => {
  switch (status) {
    ...
    case "at-risk":
      return { label: "At Risk", className: "bg-amber-500/10 text-amber-500 border-amber-500/20" };
    case "dormant":
      return { label: "Dormant", className: "bg-orange-500/10 text-orange-500 border-orange-500/20" };
    case "churned":
      return { label: "Churned", className: "bg-red-500/10 text-red-500 border-red-500/20" };
  }
};

// Helper to compute performance score (0-100)
const getPerformanceScore = (user: User): number => {
  const trustScore = user.trust_score ?? 50;
  const earnings = user.wallets?.total_earned || 0;
  const hasSocialAccounts = (user.social_accounts?.length || 0) > 0;

  // Base score from trust
  let score = trustScore;
  // Bonus for having social accounts
  if (hasSocialAccounts) score = Math.min(100, score + 10);
  // Bonus for earnings
  if (earnings > 0) score = Math.min(100, score + Math.min(20, earnings / 50));

  return Math.round(score);
};

// Helper to compute risk score (0-100, higher = more risky)
const getRiskScore = (user: User): number => {
  const trustScore = user.trust_score ?? 50;
  const hasSocialAccounts = (user.social_accounts?.length || 0) > 0;
  const hasApprovedDemo = user.social_accounts?.some(acc =>
    acc.demographic_submissions?.some(d => d.status === "approved")
  );

  // Base risk is inverse of trust
  let risk = 100 - trustScore;
  // Lower risk if has social accounts
  if (hasSocialAccounts) risk = Math.max(0, risk - 10);
  // Lower risk if has approved demographics
  if (hasApprovedDemo) risk = Math.max(0, risk - 15);

  return Math.round(risk);
};

// Helper to generate sparkline data (simplified)
const generateSparkline = (user: User): number[] => {
  const baseValue = user.wallets?.total_earned || 0;
  // Generate 7 data points with some variation
  return Array.from({ length: 7 }, (_, i) => {
    const variation = (Math.sin(i * 0.8) + 1) * 0.3 + 0.4; // Creates a wave pattern
    return Math.max(0, baseValue * variation * (0.5 + Math.random() * 0.5));
  });
};
...
export default function UsersContent() {
  ...
  const applyDatabaseFilters = (query: any, skipSearch = false) => {
    ...
    if (debouncedSearch && !skipSearch) {
      // Escape special characters that could break the filter syntax
      const term = debouncedSearch.toLowerCase().replace(/[%_\\,()]/g, char => `\\${char}`);
        ...
        <div className="flex-1 overflow-auto p-6 space-y-5">
          <Tabs defaultValue="users" className="space-y-5">
            ...
            {loading ? (
              ...
              <div>
                {/* Table Header */}
                <div className="grid grid-cols-[1fr_80px_100px_100px_100px_90px_80px_60px] gap-3 px-4 py-3 bg-muted/30 border-b border-border/30 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                ...
                {users.map(user => {
                  ...
                  const status = getRetentionStatus(user);
                  const statusBadge = getStatusBadge(status);
                  const performanceScore = getPerformanceScore(user);
                  const riskScore = getRiskScore(user);
                  const sparklineData = generateSparkline(user);

                  return (
                    <div
                      key={user.id}
                      onClick={() => openUserDetails(user)}
                      className="grid grid-cols-[1fr_80px_100px_100px_100px_90px_80px_60px] gap-3 items-center px-4 py-3 cursor-pointer transition-colors hover:bg-muted/40"
                    >
                      {/* User */}
                      <div className="min-w-[180px] flex items-center gap-3 min-w-0">
                        {user.avatar_url ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className={cn(
                          "text-xs font-medium w-7 text-right",
                          performanceScore >= 70 ? "text-emerald-500" :
                          performanceScore >= 40 ? "text-amber-500" : "text-red-400"
                        )}>
                          {performanceScore}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Progress
                          ...
                          className={cn(
                            ...
                            riskScore >= 40 ? "[&>div]:bg-amber-500" : "[&>div]:bg-emerald-500"
                          )}
                        />
                        <span className={cn(
                          "text-xs font-medium w-7 text-right",
                        ...
                        {user.social_accounts && user.social_accounts.length > 0 ? (
                          <div className="flex flex-wrap gap-1 justify-center">
                            {user.social_accounts.slice(0, 3).map(acc => {
                              const demoStatus = acc.demographic_submissions?.[0]?.status;
                              return (
                                <div key={acc.id} className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-muted/50 text-xs">
                                  {getPlatformIcon(acc.platform)}
                                  {demoStatus === "approved" && <BadgeCheck className="h-3 w-3 text-emerald-500" />}
                                  {demoStatus === "pending" && <Clock className="h-3 w-3 text-amber-500" />}
                                </div>
                              );
                            })}
                      ...
                      {/* Trend */}
                      <div className="flex justify-center">
                        <Sparklines data={sparklineData} width={60} height={20} margin={2}>
                          <SparklinesLine
                            color={totalEarned > 0 ? "#10b981" : "#6b7280"}
