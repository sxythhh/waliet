...
export function TemplateSelector({
  ...
  const fetchTemplates = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("blueprint_templates").select("*").eq("is_active", true).order("title", {
      ascending: true
    });
    if (error) {
      ...
      setTemplates((data || []).map(t => ({
        ...
        dos_and_donts: t.dos_and_donts as {
          dos: string[];
          donts: string[];
        } || {
          dos: [],
        ...
        target_personas: t.target_personas as Array<{
          name: string;
          target_audience: string;
          description: string;
        }> || [],
        assets: t.assets as Array<{
          link: string;
          notes: string;
        }> || [],
        example_videos: t.example_videos as Array<{
          url: string;
          description: string;
        }> || []
      })));
    ...
    return <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-4">
            {/* Start Blank Option */}
            <button onClick={handleStartBlank} className="w-full text-left p-4 rounded-xl border border-border/50 bg-card/30 hover:bg-card/50 hover:border-border transition-all group mb-3">
            ...
            {loading ? <div className="space-y-3">
              ...
              </div> : templates.length === 0 ? null : <div className="space-y-3">
                <p className="text-xs text-muted-foreground font-inter tracking-[-0.5px] uppercase px-1">
                  Or start from a template
                </p>
                {templates.map(template => <button key={template.id} onClick={() => handleSelect(template)} className="w-full text-left p-4 rounded-xl border border-border/50 bg-card/30 hover:bg-card/50 hover:border-border transition-all group">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-medium text-[15px] text-foreground font-inter tracking-[-0.5px]">
                          ...
                          {template.category && <Badge variant="secondary" className="text-[10px]">
                              {template.category}
                            </Badge>}
                        </div>
                        {template.description && <p className="text-sm text-muted-foreground line-clamp-2 font-inter tracking-[-0.5px]">
                            {template.description}
                          </p>}
                        {template.platforms && template.platforms.length > 0 && <div className="flex items-center gap-1.5 mt-2">
                            {template.platforms.map(platform => <Badge key={platform} variant="outline" className="text-[10px] capitalize">
                                {platform}
                              </Badge>)}
                          </div>}
  ...
  return <>
      <Button variant="outline" onClick={() => setOpen(true)} className="gap-2 border-dashed border-border/50 bg-card/30 hover:bg-card/50 text-muted-foreground hover:text-foreground font-inter tracking-[-0.5px]">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-4">
            {loading ? <div className="space-y-3">
                {[1, 2, 3].map(i => <Skeleton key={i} className="h-24 rounded-xl" />)}
              </div> : templates.length === 0 ? <div className="text-center py-12 text-muted-foreground">
