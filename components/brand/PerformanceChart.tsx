...
export interface MetricsData {
  ...
  dailyVideos: number;
  // Additional metrics for All Programs view
  spent?: number;
  creators?: number;
}
...
interface PerformanceChartProps {
  ...
  /** When true, hides metric toggles and daily/cumulative controls - shows only the selected metric */
...
export function PerformanceChart({
  ...
  // Sync with defaultMetric when it changes (from clicking stat cards)
  useEffect(() => {
  ...
  const getChartDataKey = (metric: MetricType) => {
    // Spent and creators don't have daily variants - always use the base metric
  ...
  // Generate empty placeholder data when no metrics exist
  ...
  return <Card className="p-4 sm:p-5 bg-card/30 border border-border dark:border-transparent">
      ...
      {!singleMetricMode && (
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-5">
          ...
          <div className="flex items-center gap-0 bg-transparent rounded-lg p-0.5 self-start sm:self-auto">
            <button onClick={() => setChartMode('daily')} className={`px-2 sm:px-3 py-1 sm:py-1.5 text-xs font-medium font-inter tracking-[-0.5px] rounded-md transition-all ${chartMode === 'daily' ? 'bg-[#0a0a0a] dark:bg-[#131313] text-white' : 'text-muted-foreground hover:text-foreground'}`}>
              Daily
            </button>
            <button onClick={() => setChartMode('cumulative')} className={`px-2 sm:px-3 py-1 sm:py-1.5 text-xs font-medium font-inter tracking-[-0.5px] rounded-md transition-all ${chartMode === 'cumulative' ? 'bg-[#0a0a0a] dark:bg-[#131313] text-white' : 'text-muted-foreground hover:text-foreground'}`}>
              Cumulative
            </button>
          </div>
        </div>
      ...
      <div className="h-56 sm:h-72 relative">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{
            ...
            <defs>
              {(['views', 'likes', 'shares', 'bookmarks', 'videos', 'spent', 'creators'] as MetricType[]).map(metric => <linearGradient key={metric} id={`gradient-${metric}`} x1="0" y1="0" x2="0" y2="1">
