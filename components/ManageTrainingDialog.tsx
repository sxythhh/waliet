      ...
      if (coursesData && coursesData.length > 0) {
        ...
        modulesData?.forEach(module => {
          ...
          const typedModule: Module = {
            ...
            assets: Array.isArray(module.assets) ? module.assets as Array<{
              title: string;
              url: string;
            }> : []
          };
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-background border-border">
        ...
        <div className="space-y-4">
          ...
          {courses.map(course => {
          const courseData = editingCourse[course.id] || {
            title: course.title,
            description: course.description
          };
          return <Card key={course.id} className="bg-card border-border">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        {course.banner_url && <div className="relative w-full h-32 rounded-lg overflow-hidden bg-muted">
                    ...
                    }} disabled={uploadingBanner === course.id} className="bg-muted border-border text-foreground file:text-muted-foreground" />
                      </div>
                      
                      <Input value={courseData.title || ""} onChange={e => updateCourseField(course.id, 'title', e.target.value)} onBlur={() => saveCourse(course.id)} className="bg-muted border-border text-foreground font-semibold" placeholder="Course Title" />
                      <Textarea value={courseData.description || ""} onChange={e => updateCourseField(course.id, 'description', e.target.value)} onBlur={() => saveCourse(course.id)} className="bg-muted border-border text-foreground" placeholder="Course Description" />
                    </div>
                    <Button size="icon" variant="ghost" onClick={() => setExpandedCourse(expandedCourse === course.id ? null : course.id)} className="text-muted-foreground">
                ...
                {expandedCourse === course.id && <CardContent className="space-y-4">
                  ...
                  return <AccordionItem key={module.id} value={module.id} className="bg-muted border-border rounded-lg px-4">
                            <div className="flex items-center gap-2">
                            ...
                            <AccordionContent className="space-y-4 pt-2 pb-4">
                              <div className="space-y-2">
                                ...
                                <Input value={moduleData.title || ""} onChange={e => updateModuleField(module.id, 'title', e.target.value)} onBlur={() => saveModule(module.id)} className="bg-card border-border text-foreground" placeholder="Enter module title" />
                              </div>
                              
                              <div className="space-y-2">
                                <Label className="text-foreground text-sm">Video Embed (YouTube, Vimeo, Wistia, etc.)</Label>
                                <Textarea value={moduleData.video_url || ""} onChange={e => updateModuleField(module.id, 'video_url', e.target.value)} onBlur={() => saveModule(module.id)} className="bg-card border-border text-foreground font-mono text-xs" placeholder="Paste embed code or URL here..." rows={4} />
                              ...
                              <div className="space-y-2">
                                ...
                                {(moduleData.assets || []).map((asset, assetIndex) => <div key={assetIndex} className="flex gap-2">
                                    <Input value={asset.title} onChange={e => {
                          ...
                          }} onBlur={() => saveModule(module.id)} className="bg-card border-border text-foreground" placeholder="Asset title" />
                                    <Input value={asset.url} onChange={e => {
                            const newAssets = [...(moduleData.assets || [])];
                            newAssets[assetIndex] = {
                              ...asset,
                              url: e.target.value
                            ...
                            updateModuleField(module.id, 'assets', newAssets);
                          }} onBlur={() => saveModule(module.id)} className="bg-card border-border text-foreground" placeholder="https://..." />
                          ...
                          updateModuleField(module.id, 'assets', newAssets);
                              ...
                              <div className="space-y-2">
                                ...
                                <RichTextEditor content={moduleData.content || ""} onChange={content => {
                          updateModuleField(module.id, 'content', content);
                        }} placeholder="Write your course content here..." />
                                <Button size="sm" onClick={() => saveModule(module.id)} disabled={loading} className="mt-2">
                                  Save Content
                                </Button>
                              </div>
                            </AccordionContent>
