...
export function BuyHoursModal({
  ...
  const handlePurchase = async () => {
    ...
    try {
      // Create the checkout config and plan on our backend
      ...
      if (!data.purchaseUrl) {
        throw new Error("No purchase URL returned");
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="md">
        ...
        {/* Seller Info */}
        <div className="flex items-center gap-3 p-4 bg-muted/50 rounded-xl overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-medium truncate">{seller.name || "Seller"}</p>
