...
export function SubmissionHeatmap({
  ...
  const {
    ...
    submissions.forEach(sub => {
      ...
      day.total++;
      if (sub.status === "approved") day.approved++;else if (sub.status === "pending") day.pending++;else if (sub.status === "rejected") day.rejected++;
    });
  ...
  const getColorClass = (day: SubmissionDay, isFuture: boolean, isToday: boolean) => {
    ...
    if (isToday) return day.total === 0 ? "bg-primary/20 ring-1 ring-primary/40" : day.approved > 0 ? "bg-emerald-500/80 ring-1 ring-emerald-400" : day.pending > 0 ? "bg-amber-500/70 ring-1 ring-amber-400" : "bg-primary/20 ring-1 ring-primary/40";
  ...
  return <TooltipProvider delayDuration={100}>
      <div className="w-full">
        ...
        <div className="flex items-center gap-1 mb-3">
          {(["week", "month", "3months"] as TimeRange[]).map(range => <Button key={range} variant="ghost" size="sm" onClick={() => setTimeRange(range)} className={`h-6 px-2 text-[10px] font-inter tracking-[-0.5px] ${timeRange === range ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}>
        ...
        <div className="flex mb-1.5 pl-6">
          ...
          return <div key={idx} className="text-[10px] text-muted-foreground font-inter tracking-[-0.5px]" style={{
        ...
        <div className="flex gap-[2px]">
          ...
          <div className="flex flex-col gap-[2px] pr-1">
            {dayLabels.map((label, idx) => <div key={idx} className="h-[12px] w-4 text-[9px] text-muted-foreground font-inter tracking-[-0.5px] flex items-center justify-end pr-0.5">
                {idx % 2 === 0 ? label : ""}
              </div>)}
          </div>
          ...
          <div className="flex gap-[2px] flex-1">
            {weeks.map((week, weekIdx) => <div key={weekIdx} className="flex flex-col gap-[2px] flex-1">
              ...
              return <Tooltip key={dayIdx}>
                  ...
                  }} className={`h-[12px] w-full min-w-[8px] rounded-[3px] transition-all hover:scale-110 cursor-pointer ${getColorClass(day, isFuture, isToday)}`} style={isSelected ? {
                      ...
                      <TooltipContent side="top" className="bg-popover border border-border/60 shadow-lg px-3 py-2">
                        <div className="font-geist tracking-[-0.5px]">
                          ...
                          {isFuture ? <p className="text-sm font-medium text-foreground/60">Upcoming</p> : <>
                              ...
                              {day.total > 0 && <div className="flex gap-2 mt-1 text-[10px]">
                                  {day.approved > 0 && <span className="text-emerald-500">{day.approved} approved</span>}
                                  {day.pending > 0 && <span className="text-amber-500">{day.pending} pending</span>}
                                  {day.rejected > 0 && <span className="text-red-400">{day.rejected} rejected</span>}
                                </div>}
                            </>}
                        </div>
                      </TooltipContent>
