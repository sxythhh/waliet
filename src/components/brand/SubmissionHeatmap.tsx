        ...
        <div className="flex items-center gap-1 mb-3">
          {(["week", "month", "3months"] as TimeRange[]).map(range => (
            <Button 
              ...
              className={cn(
                "h-6 px-2 text-[10px] font-medium tracking-[-0.3px] rounded-md",
                timeRange === range 
        ...
        <div className="flex mb-1.5 pl-6">
          {monthLabels.map((month, idx) => {
            ...
            return (
              <div 
                key={idx} 
                className="text-[10px] text-muted-foreground font-medium tracking-[-0.3px]" 
        ...
        <div className="flex gap-[2px]">
          ...
          <div className="flex flex-col gap-[2px] pr-1">
            {dayLabels.map((label, idx) => (
              <div 
                key={idx} 
                className="h-[12px] w-4 text-[9px] text-muted-foreground font-medium tracking-[-0.3px] flex items-center justify-end pr-0.5"
              >
                {idx % 2 === 0 ? label : ""}
              </div>
            ))}
