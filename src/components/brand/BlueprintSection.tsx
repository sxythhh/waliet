...
export function BlueprintSection({
  ...
  return <div ref={setNodeRef} style={style} className={cn("group relative rounded-xl border border-border/50 bg-card/30 transition-all hover:border-border/80", isDragging && "opacity-50 z-50 shadow-xl")}>
      ...
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
        ...
        <span className="text-sm font-medium font-inter tracking-[-0.5px] text-foreground">
          {title}
        </span>
        {/* Drag Handle - inline after title, hidden on mobile */}
        {!isMobile && <div {...attributes} {...listeners} className="opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing" onClick={e => e.stopPropagation()}>
