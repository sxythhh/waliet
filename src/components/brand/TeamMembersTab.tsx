...
export function TeamMembersTab({
  ...
  const fetchTeamData = async () => {
    try {
      ...
      } = await supabase.from("brand_invitations").select("*").eq("brand_id", brandId).eq("status", "pending");
  ...
  return <div className="space-y-6">
        ...
        <div className="space-y-1">
          {members.map(member => <div key={member.id} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                {canManageTeam && member.role !== "owner" && <Button variant="ghost" size="icon" onClick={() => handleRemoveMember(member.id)} className="h-7 w-7 text-muted-foreground hover:text-destructive">
      ...
      {invitations.length > 0 && <div className="space-y-2">
          ...
          <div className="space-y-1">
            {invitations.map(invitation => <div key={invitation.id} className="flex items-center justify-between p-2.5 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {canManageTeam && <Button variant="ghost" size="sm" onClick={() => handleCancelInvitation(invitation.id)} className="h-7 text-xs font-inter tracking-[-0.5px] text-muted-foreground hover:text-red-500 hover:bg-red-500/10">
                      Cancel
                    </Button>}
                </div>
              </div>)}
