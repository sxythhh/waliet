              ...
              return filteredVids.map(video => {
                ...
                return <div key={video.id} className={cn("group relative w-full rounded-2xl border bg-gradient-to-br from-card/80 to-card/40 backdrop-blur-sm overflow-hidden hover:shadow-lg transition-all duration-300 font-sans tracking-[-0.5px]", isSelected ? "border-primary/50 bg-primary/5" : "border-border/30 hover:border-border/60")}>
                      ...
                      {video.status === "pending" && <div className="flex items-center gap-1.5 px-3 pb-3 pt-1">
                          <Button size="sm" className="h-7 flex-1 text-[11px] font-medium gap-1 bg-[#1c823a] hover:bg-[#1c823a]/90 text-white rounded-md shadow-none" onClick={() => handleApprove(video)} disabled={processing}>
                            <Check className="h-3 w-3" />
                            Approve
                          </Button>
                          <Button size="sm" className="h-7 flex-1 text-[11px] font-medium gap-1 bg-[#b60b0b] hover:bg-[#b60b0b]/90 text-white rounded-md shadow-none" onClick={() => {
                      setSelectedSubmission(video);
                      setRejectDialogOpen(true);
                    }} disabled={processing}>
                            <X className="h-3 w-3" />
