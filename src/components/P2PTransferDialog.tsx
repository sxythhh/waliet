...
export function P2PTransferDialog({ open, onOpenChange, currentBalance, onTransferComplete }: P2PTransferDialogProps) {
  const [recipientIdentifier, setRecipientIdentifier] = useState("");
  const [amount, setAmount] = useState("");
  const [note, setNote] = useState("");
  const [isValidating, setIsValidating] = useState(false);
  const [isTransferring, setIsTransferring] = useState(false);
  const [recipientInfo, setRecipientInfo] = useState<{ id: string; username: string; email: string } | null>(null);
  ...
  const handleValidateRecipient = async () => {
    ...
    try {
      ...
      const { data: profiles, error } = await supabase
        ...
        .or(`username.eq.${recipientIdentifier},email.eq.${recipientIdentifier}`)
        .limit(1);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const { error: senderTxnError } = await supabase
      ...
      const { error: recipientTxnError } = await supabase
        .from("wallet_transactions")
        .insert({
      ...
      <DialogContent className="bg-black border-0 text-white max-w-md">
        ...
        <DialogFooter className="flex gap-2">
          ...
          <Button
            ...
            disabled={!recipientInfo || !amount || Number(amount) <= 0 || isTransferring || isValidating}
