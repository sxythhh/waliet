...
export function JoinCampaignSheet({
  ...
  const loadSocialAccounts = async () => {
    ...
    } = await supabase.from("social_account_campaigns").select("social_account_id").eq("campaign_id", campaign.id);
  ...
  const handleSubmit = async () => {
    ...
    if (campaign.requires_application !== false && campaign.application_questions?.length > 0) {
      const unansweredQuestions = campaign.application_questions.filter((q, idx) => !answers[idx]?.trim());
    ...
    try {
      ...
      const successMessage = campaign.requires_application === false ? "Successfully joined the campaign! This account is now connected." : "Application submitted successfully! This account is now connected to this campaign.";
        ...
        <div className="mt-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {campaign.platforms.map(platform => <span key={platform} className="px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium capitalize">
                  {platform}
                </span>)}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            {socialAccounts.length === 0 ? <div className="p-4 rounded-lg bg-muted/50 text-center space-y-2">
              ...
              return <button key={account.id} type="button" onClick={() => setSelectedAccount(account.id)} className={`flex items-center gap-3 p-3 rounded-lg border-2 transition-all ${isSelected ? "border-blue-500 bg-blue-500/10" : "border-border hover:border-muted-foreground/50 bg-card"}`}>
                      {platformIcon && <div className="w-10 h-10 rounded-lg bg-muted flex items-center justify-center flex-shrink-0">
          ...
          {campaign.requires_application !== false && campaign.application_questions?.map((question, index) => <div key={index} className="space-y-2">
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button className="flex-1" onClick={handleSubmit} disabled={submitting || !selectedAccount}>
