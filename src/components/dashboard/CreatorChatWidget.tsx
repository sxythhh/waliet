...
export function CreatorChatWidget() {
  ...
  const mailIcon = mailIconLight; // Always use white icon on blue button background
  ...
  return <>
      ...
      <button onClick={() => setIsOpen(true)} className="md:hidden fixed bottom-20 right-4 z-50 w-14 h-14 rounded-full bg-primary hover:bg-[#1a50c0] transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center group">
      ...
      {isOpen && <div className="fixed bottom-20 right-4 md:bottom-6 md:right-6 z-50 w-[calc(100vw-2rem)] max-w-[380px] h-[480px] md:h-[520px] bg-background border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-200">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-card/50">
            {activeConversation ? <div className="flex items-center gap-3">
              ...
              </div> : activeAnnouncement ? <div className="flex items-center gap-3">
                <button onClick={handleBack} className="text-muted-foreground hover:text-foreground transition-colors">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M19 12H5M12 19l-7-7 7-7" />
                  </svg>
                ...
                <div className="p-4">
                  <div className="p-4 bg-amber-500/10 rounded-xl border border-amber-500/20">
                    ...
                    <p className="text-xs text-muted-foreground mt-3">
                      {format(new Date(activeAnnouncement.created_at), "MMM d, yyyy 'at' h:mm a")}
                    </p>
                  </div>
                </div>
              </ScrollArea>
            ) : !activeConversation ? (
              // Conversations List with Announcements
              <ScrollArea className="h-full">
                {loading ? (
                  <div className="flex items-center justify-center h-full min-h-[400px]">
                    <div className="w-6 h-6 border-2 border-primary border-t-transparent rounded-full animate-spin" />
                  </div>
                ) : announcements.length === 0 && conversations.length === 0 ? (
                  // Empty State
