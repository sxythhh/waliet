...
export function SubmissionsTab() {
  ...
  const calculatePayoutSummary = () => {
    let accruing = { amount: 0, videoCount: 0 };
    let clearing = { 
      amount: 0, 
      videoCount: 0, 
    ...
    let paid = { amount: 0, videoCount: 0 };
  ...
  const handleRequestVideoPayout = async () => {
    ...
    try {
      ...
      setVideoPayoutDialog({ open: false, submission: null });
      fetchSubmissions();
      refetchLedger();
    } catch (error) {
  ...
  return (
    <Card className="bg-card border rounded-xl overflow-hidden border-[#141414]/0">
        ...
        <DropdownMenu open={filterOpen} onOpenChange={open => {
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="outline" className="gap-2 rounded-[9px] border-border bg-background hover:bg-background px-4 py-2 h-auto">
              ...
              <span className="font-medium" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Filter</span>
          ...
          <DropdownMenuContent align="start" className="w-[280px] p-2.5 overflow-hidden bg-white dark:bg-[#0a0a0a] border-[#dce1eb] dark:border-[#141414]" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>
            <div className="relative">
              ...
              <div className={`transition-all duration-200 ease-out ${filterSubmenu === 'main' ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full absolute inset-0'}`}>
                ...
                {/* Clear Filters */}
                {hasActiveFilters && (
                  <button onClick={() => { setStatusFilter('all'); setTypeFilter('all'); setProgramFilter('all'); }} className="w-full mt-3 pt-3 border-t border-border text-sm text-muted-foreground hover:text-foreground transition-colors text-center">
                    Clear all filters
                  </button>
                )}
              </div>
              ...
              <div className={`transition-all duration-200 ease-out ${filterSubmenu === 'status' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full absolute inset-0'}`}>
                ...
                <div className="space-y-1">
                  {[{ value: 'all', label: 'All Statuses' }, { value: 'pending', label: 'Pending' }, { value: 'approved', label: 'Approved' }, { value: 'rejected', label: 'Rejected' }].map(option => (
                    <button key={option.value} onClick={() => { setStatusFilter(option.value); setFilterSubmenu('main'); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${statusFilter === option.value ? 'bg-muted' : 'hover:bg-muted/50'}`}>
              ...
              <div className={`transition-all duration-200 ease-out ${filterSubmenu === 'type' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full absolute inset-0'}`}>
                ...
                <div className="space-y-1">
                  {[{ value: 'all', label: 'All Types' }, { value: 'campaign', label: 'Campaign' }, { value: 'boost', label: 'Boost' }].map(option => (
                    <button key={option.value} onClick={() => { setTypeFilter(option.value); setFilterSubmenu('main'); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${typeFilter === option.value ? 'bg-muted' : 'hover:bg-muted/50'}`}>
                      <span className="text-sm">{option.label}</span>
              ...
              <div className={`transition-all duration-200 ease-out ${filterSubmenu === 'program' ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-full absolute inset-0'}`}>
                ...
                <div className="space-y-1 max-h-[200px] overflow-y-auto">
                  <button onClick={() => { setProgramFilter('all'); setFilterSubmenu('main'); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${programFilter === 'all' ? 'bg-muted' : 'hover:bg-muted/50'}`}>
                    <span className="text-sm">All Programs</span>
                    {programFilter === 'all' && <Check className="h-4 w-4 ml-auto" />}
                  </button>
                  {uniquePrograms.map(program => (
                    <button key={program.id} onClick={() => { setProgramFilter(program.id); setFilterSubmenu('main'); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-colors ${programFilter === program.id ? 'bg-muted' : 'hover:bg-muted/50'}`}>
                      {program.brand_logo_url ? (
      ...
      <RequestVideoPayoutDialog
        open={videoPayoutDialog.open}
        onOpenChange={(open) => setVideoPayoutDialog({ open, submission: open ? videoPayoutDialog.submission : null })}
        videoTitle={videoPayoutDialog.submission?.video_title || 'Untitled Video'}
        programName={videoPayoutDialog.submission?.program.title || ''}
        amount={videoPayoutDialog.submission?.estimated_payout || 0}
        onConfirm={handleRequestVideoPayout}
        isLoading={requestingVideoPayout}
      />
      ...
      <div className="pb-6 px-0">
        {submissions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-[#dce1eb] dark:border-[#141414] hover:bg-transparent">
                    <TableHead className="text-foreground font-medium text-sm h-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Video</TableHead>
                    ...
                    <TableHead className="text-foreground font-medium text-sm h-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Account</TableHead>
                    <TableHead className="text-foreground font-medium text-sm h-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Status</TableHead>
                    <TableHead className="text-foreground font-medium text-sm h-12 text-right" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Views</TableHead>
                    <TableHead className="text-foreground font-medium text-sm h-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Submitted</TableHead>
                    <TableHead className="text-foreground font-medium text-sm h-12 text-right" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Payout</TableHead>
                    <TableHead className="text-foreground font-medium text-sm h-12 w-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}></TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      <TableRow key={submission.id} className="hover:bg-[#fafafa] dark:hover:bg-[#0a0a0a] transition-colors border-[#dce1eb] dark:border-[#141414]">
                          ...
                          <a href={submission.video_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 group">
                            <div className="w-10 h-[60px] rounded-lg overflow-hidden bg-muted flex-shrink-0 relative">
                              {submission.video_cover_url ? (
                                <img src={submission.video_cover_url} alt={submission.video_title || 'Video'} className="w-full h-full object-cover" />
                              ) : (
                            ...
                            <div className="min-w-0 max-w-[200px]">
                              <p className="text-sm font-medium truncate group-hover:underline transition-colors" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                        ...
                        <TableCell className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium truncate max-w-[120px]" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                        ...
                        <TableCell className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-foreground" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span className="text-sm font-medium tabular-nums cursor-default" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                        ...
                        <TableCell className="py-3">
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span className="text-sm text-muted-foreground underline decoration-dotted cursor-pointer hover:text-foreground" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                        ...
                        <TableCell className="py-3">
                          <PayoutStatusBadge
                            ...
                            onRequestPayout={() => setVideoPayoutDialog({ open: true, submission })}
