...
export function SubmissionsTab() {
  ...
  const uniquePrograms = Array.from(
    new Map(submissions.map(s => [s.program.id, s.program])).values()
  );
      ...
      <div className="pt-5 pb-4 px-0">
        <DropdownMenu open={filterOpen} onOpenChange={(open) => {
          ...
          <DropdownMenuTrigger asChild>
            <Button 
              variant="outline" 
              className="gap-2 rounded-[9px] border-border bg-background hover:bg-background px-4 py-2 h-auto"
            >
              <SlidersHorizontal className="h-4 w-4" />
              <span className="font-medium" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Filter</span>
              {hasActiveFilters && (
                <span className="w-2 h-2 rounded-full bg-[#2060df]" />
              )}
          ...
          <DropdownMenuContent 
            align="start" 
            className="w-[280px] p-2.5 overflow-hidden bg-white dark:bg-[#0a0a0a] border-[#dce1eb] dark:border-[#141414]"
            ...
            <div className="relative">
              ...
              <div className={`transition-all duration-200 ease-out ${filterSubmenu === 'main' ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-full absolute inset-0'}`}>
                ...
                <div className="space-y-1">
                  {/* Status Filter */}
                  <button
                    onClick={() => setFilterSubmenu('status')}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {statusFilter !== 'all' && (
                        <span className="text-xs text-muted-foreground capitalize">{statusFilter}</span>
                      )}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {typeFilter !== 'all' && (
                        <span className="text-xs text-muted-foreground capitalize">{typeFilter}</span>
                      )}
                      <ChevronDown className="h-4 w-4 -rotate-90" />
                    </div>
                  </button>
                  ...
                  {/* Program Filter */}
                  <button
                    onClick={() => setFilterSubmenu('program')}
                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-muted/50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-[#dce1eb] dark:border-[#141414] hover:bg-transparent">
                    ...
                    <TableHead className="text-foreground font-medium text-sm h-12" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>Reviewed</TableHead>
                    ...
                    >
                      ...
                      <TableCell className="py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground capitalize" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                            {submission.platform}
                          </span>
                        </div>
                      </TableCell>
                      ...
                      {/* Reviewed */}
                      <TableCell className="py-4 text-sm text-muted-foreground" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                        {submission.reviewed_at ? (
                          ...
                          format(new Date(submission.updated_at), 'MMM d')
                        ) : (
                      ...
                      <TableCell className="py-4 text-right">
                        {submission.status === 'approved' && submission.estimated_payout ? (
                          <span className="text-sm font-semibold text-green-600 dark:text-green-400" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>
                            ${submission.estimated_payout.toFixed(2)}
                          </span>
                        ) : submission.status === 'pending' && submission.estimated_payout ? (
                          <span className="text-sm text-muted-foreground" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>
                            ~${submission.estimated_payout.toFixed(2)}
                          </span>
                        ) : (
