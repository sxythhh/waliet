  ...
  const handleTransfer = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke("p2p-transfer", {
        body: {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="font-inter tracking-[-0.5px]">
            {step === "input" ? "Send money to another user. A 3% fee applies to all transfers." : "Please review the transfer details before confirming."}
          </DialogDescription>
        </DialogHeader>

        {step === "input" ? <div className="space-y-4 py-2">
            <div className="space-y-2">
              <Label htmlFor="username" className="font-inter tracking-[-0.5px]">Recipient Username</Label>
              <Input id="username" placeholder="Enter username" value={username} onChange={e => setUsername(e.target.value)} className="font-inter tracking-[-0.5px]" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="amount" type="number" min={MINIMUM_TRANSFER} max={currentBalance} step="0.01" placeholder="0.00" value={amount} onChange={e => setAmount(e.target.value)} className="pl-7 font-inter tracking-[-0.5px]" />
              </div>
            ...
            {parsedAmount >= MINIMUM_TRANSFER && <div className="rounded-lg bg-muted/50 p-3 space-y-1">
            ...
            <Button onClick={handleContinue} disabled={!isValidAmount || !isValidUsername} className="w-full font-inter tracking-[-0.5px]">
              Continue
