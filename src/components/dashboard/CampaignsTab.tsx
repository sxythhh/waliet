...
export function CampaignsTab() {
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      {campaigns.map(campaign => <Card key={campaign.id} className={`bg-gradient-card border-0 transition-all duration-300 overflow-hidden ${campaign.submission_status === 'approved' ? 'cursor-pointer hover:scale-[1.02]' : 'cursor-not-allowed opacity-50 grayscale'}`} onClick={() => {
      if (campaign.submission_status === 'approved') {
        navigate(`/campaign/${campaign.id}`);
          ...
          <div className="relative h-48 bg-gradient-to-br from-background to-muted overflow-hidden">
            {campaign.banner_url ? <img src={campaign.banner_url} alt={campaign.title} className="w-full h-full object-cover" /> : <div className="absolute inset-0 flex items-center justify-center">
                <img src={campaign.brand_logo_url} alt={campaign.brand_name} className="w-24 h-24 rounded-2xl object-cover border-0" />
              </div>}
            <div className="absolute top-4 right-4 flex gap-2">
              {campaign.submission_status && <Badge className={`font-bold text-base px-3 py-1 ${campaign.submission_status === 'pending' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' : campaign.submission_status === 'approved' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`} variant="outline">
