...
export function PayoutStatusBadge({
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      {/* Amount */}
      <span 
        className={`text-sm font-semibold ${config.className}`}
      ...
      <div className="flex items-center gap-1.5">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${config.bgClass} ${config.className}`}>
                ...
                {status === 'clearing' && clearingEndsAt ? (
                  <ClearingCountdown endsAt={clearingEndsAt} showIcon={false} compact />
                ) : (
                  config.label
                )}
              </span>
            ...
            <TooltipContent side="left" className="max-w-xs">
              ...
              {status === 'locked' && (
                <p className="text-sm">Payout requested, waiting to enter clearing period.</p>
              )}
              {status === 'clearing' && (
                <p className="text-sm">In 7-day clearing period. Brands can flag during first 4 days.</p>
              )}
              {status === 'paid' && (
                <p className="text-sm">Successfully paid to your wallet.</p>
              )}
              {status === 'clawed_back' && (
                <p className="text-sm">This payment was flagged and revoked by the brand.</p>
              )}
              {status === 'pending_approval' && (
                <p className="text-sm">Waiting for brand approval. Amount is estimated.</p>
              )}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
        ...
        {status === 'accruing' && canRequest && onRequestPayout && (
          <Button
            onClick={(e) => {
              e.stopPropagation();
              onRequestPayout();
            ...
            variant="ghost"
            size="sm"
            className="h-5 px-2 text-xs text-green-600 dark:text-green-400 hover:bg-green-500/10"
          >
            Request
          </Button>
        )}
