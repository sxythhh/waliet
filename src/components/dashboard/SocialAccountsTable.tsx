...
interface SocialAccountsTableProps {
  ...
  onVerifyAccount: (account: { id: string; platform: string; username: string }) => void;
  onSubmitDemographics: (account: { id: string; platform: string; username: string }) => void;
}
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b border-[#dce1eb] dark:border-[#141414] hover:bg-transparent dark:bg-[#0e0e0e]">
            <TableHead className="text-foreground font-medium text-sm h-12">Account</TableHead>
            <TableHead className="text-foreground font-medium text-sm h-12">Followers</TableHead>
            <TableHead className="text-foreground font-medium text-sm h-12">Campaigns</TableHead>
            <TableHead className="text-foreground font-medium text-sm h-12">Demographics</TableHead>
            <TableHead className="text-foreground font-medium text-sm h-12">Status</TableHead>
            <TableHead className="text-foreground font-medium text-sm h-12 text-right">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!account.is_verified && (
                          <button
                            onClick={() => onVerifyAccount({ id: account.id, platform: account.platform, username: account.username })}
                  ...
                  {connectedCampaigns.length > 0 ? (
                    <Popover>
                      <PopoverTrigger asChild>
                        <button className="text-sm text-primary hover:underline cursor-pointer" style={{ fontFamily: "Inter", letterSpacing: "-0.3px" }}>
                      ...
                      <PopoverContent className="w-64 p-3 bg-popover border rounded-xl z-50" align="start">
                        <div className="space-y-2">
                          ...
                          {connectedCampaigns.map(({ connection_id, campaign }) => (
                            <div
                              key={campaign.id}
                              className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors group"
                            >
                              {campaign.brand_logo_url && (
                                <img src={campaign.brand_logo_url} alt={campaign.brand_name} className="w-5 h-5 rounded object-cover" />
                              )}
                              <span className="text-sm flex-1 truncate">{campaign.title}</span>
                              <button
                                onClick={() => onUnlinkCampaign(connection_id, campaign.title)}
                ...
                <TableCell className="py-3">
                  {demographicSubmissions.length === 0 ? (
                    <button
                      onClick={() => onSubmitDemographics({ id: account.id, platform: account.platform, username: account.username })}
                    ...
                    <DemographicStatusCard
                      ...
                      onSubmitNew={() => onSubmitDemographics({ id: account.id, platform: account.platform, username: account.username })}
                      onRefresh={onRefresh}
                    />
                  )}
                </TableCell>
                ...
                <TableCell className="py-3 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <Button
                      ...
                      onClick={() => onLinkCampaign(account)}
                    ...
                    <Button
                      ...
                      title={account.hidden_from_public ? "Show on public profile" : "Hide from public profile"}
                    >
                      {account.hidden_from_public ? (
