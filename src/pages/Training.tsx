  ...
  return (
    <div className="min-h-screen p-4 md:p-8 bg-[#191919]">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-bold text-white mb-6 md:mb-8">Training Portal</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {courses.map((course) => (
            <Card 
              key={course.id} 
              className={`bg-[#202020] border-none overflow-hidden group hover:border-white/20 transition-all cursor-pointer ${course.is_locked ? 'opacity-60 cursor-not-allowed' : ''}`}
              onClick={() => {
                if (!course.is_locked) {
