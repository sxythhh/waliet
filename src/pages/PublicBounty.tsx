...
const PublicBounty = () => {
    ...
    try {
      ...
      if (bountyData.brand_id) {
        const {
          data: brandData,
          error: brandError
        } = await supabase.from("brands").select("name, logo_url, description").eq("id", bountyData.brand_id).single();
        if (!brandError) {
          setBrand(brandData);
        }
      }
  ...
  return <div className="min-h-screen bg-background">
      ...
      backgroundImage: bounty.banner_url ? `url(${bounty.banner_url})` : "linear-gradient(135deg, hsl(var(--primary)), hsl(var(--primary-foreground)))"
    }}>
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-16 pb-8">
        {/* Brand Info Card */}
        {brand && <Card className="p-4 mb-6 flex items-center gap-4 bg-card/95 backdrop-blur">
            {brand.logo_url && <img src={brand.logo_url} alt={brand.name} className="h-16 w-16 rounded-lg object-cover" />}
            <div>
        ...
        <Card className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            {bounty.description && <p className="text-muted-foreground">{bounty.description}</p>}
          </div>
          ...
          {/* Timeline */}
          {(bounty.start_date || bounty.end_date) && <div className="flex items-center gap-4 text-sm text-muted-foreground">
              <Calendar className="h-4 w-4" />
              {bounty.start_date && <span>
                  Starts: {new Date(bounty.start_date).toLocaleDateString()}
                </span>}
              {bounty.end_date && <span>
          ...
          <div className="pt-4 border-t">
            {isFull ? <div className="text-center p-4 bg-muted rounded-lg">
                <p className="text-muted-foreground">
                  This bounty campaign is currently full. All positions have been filled.
                </p>
              </div> : <Button size="lg" className="w-full" onClick={handleApplyClick}>
      ...
      {bounty.blueprint_embed_url && <div className="w-full py-8 px-4 bg-muted/30">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="w-full aspect-video rounded-lg overflow-hidden border bg-background shadow-lg">
              <iframe src={bounty.blueprint_embed_url.startsWith('http') ? bounty.blueprint_embed_url : `https://${bounty.blueprint_embed_url}`} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen title="Campaign Blueprint" />
            </div>
          </div>
        </div>}

      {bounty && <ApplyToBountySheet open={showApplySheet} onOpenChange={setShowApplySheet} bounty={bounty} onSuccess={() => {
