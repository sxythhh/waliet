...
import { ChevronRight, ExternalLink, Megaphone, LogOut, Link2, Plus, Loader2 } from "lucide-react";
...
export default function CreatorCampaignDetails() {
  ...
  // Fetch campaign data only if not passed via navigation state
  useEffect(() => {
    const fetchCampaign = async () => {
      if (!id) return;
      // Skip fetch if we already have campaign data from navigation
      if (campaign) {
        setLoading(false);
        return;
      }
  ...
  // Fetch blueprint content and assets
  useEffect(() => {
  ...
  // Build campaign data needed for sidebar components
  const sidebarCampaignData = {
    id: campaign.id,
    brand_name: campaign.brand_name,
    brand_slug: (campaign as any).brands?.slug,
        ...
        <div className="flex-1 max-w-3xl">
          ...
          <div className="flex items-start gap-3 sm:gap-4 mb-6">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {hasConnectedAccounts && campaign.payment_model === 'pay_per_post' && (
                <Button
          ...
          <div className="mb-6 p-4 rounded-2xl bg-card border border-border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-base font-semibold" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>
            ...
            {/* Demographics Reminder */}
            {nextPayout.daysUntil === 1 && (
              <div className="mt-4 p-3 rounded-xl bg-amber-500/10 border border-transparent flex items-center gap-3">
            ...
            {expectedPayout !== null && (
              <div className="mt-4 p-4 rounded-xl bg-[#f4f4f4] dark:bg-[#0f0f0f]">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-xs text-muted-foreground mb-0.5" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>Overall Expected Payout</p>
                    <p style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }} className="text-xl font-semibold text-foreground">
                      ${expectedPayout.amount.toFixed(2)}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground" style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }}>Total views</p>
                    <p className="text-sm font-medium" style={{ fontFamily: 'Inter', letterSpacing: '-0.5px' }}>
                  ...
                  return (
                    <div>
                      <div className="h-2 rounded-full overflow-hidden flex bg-muted/50">
                        <div className="bg-[#22c55e] transition-all" style={{ width: `${Math.min(paidPercent, 100)}%` }} />
                        <div
                        ...
                        />
                        <div className="bg-muted-foreground/20 transition-all" style={{ width: `${remainingPercent}%` }} />
                      </div>
                      <div style={{ fontFamily: 'Inter', letterSpacing: '-0.3px' }} className="justify-between mt-2 text-[10px] flex flex-row">
          ...
          {(blueprintContent || campaign.description) && (
            <div className="mb-6">
              <div className="relative">
                {blueprintContent ? (
                  <div
                    className={`text-sm text-foreground/90 leading-relaxed overflow-hidden transition-all prose prose-sm dark:prose-invert max-w-none break-words ${showFullDescription ? '' : 'max-h-[120px]'}`}
                  ...
                  <div
                    className={`text-sm text-foreground/90 leading-relaxed overflow-hidden transition-all whitespace-pre-line break-words ${showFullDescription ? '' : 'max-h-[120px]'}`}
                    style={{ fontFamily: 'Inter', letterSpacing: '-0.3px', overflowWrap: 'break-word', wordBreak: 'break-word' }}
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {campaign.connected_accounts!.map(account => (
                  <button
                    ...
                    className="group relative flex items-center gap-2.5 pl-3 pr-4 py-2.5 transition-all duration-200 cursor-pointer bg-popover rounded-md border border-border hover:border-border/80"
                  >
          ...
          {/* Asset Links Section */}
          {(blueprintAssets && blueprintAssets.length > 0 ? blueprintAssets : hasAssetLinks ? campaign.asset_links : null) && (
            <div className="mb-6">
                ...
                {(blueprintAssets && blueprintAssets.length > 0 ? blueprintAssets : campaign.asset_links!).map((link, index) => {
                  ...
                  return (
                    <a
                      ...
                      className="flex items-center gap-3 p-4 rounded-xl bg-[#f4f4f4] dark:bg-[#0f0f0f] hover:bg-[#e8e8e8] dark:hover:bg-[#141414] transition-colors group border border-border"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium font-inter tracking-[-0.3px] text-sm truncate">{link.label || 'Asset Link'}</p>
        ...
        <aside className="hidden lg:block w-80 flex-shrink-0">
          <div className="sticky top-6 space-y-4">
            ...
            <CampaignQuickActionsCard
              ...
              hasConnectedAccounts={hasConnectedAccounts || false}
              onSubmitVideo={() => setShowSubmitVideoDialog(true)}
              onLeaveCampaign={() => setLeaveCampaignDialogOpen(true)}
            />
          </div>
        </aside>
      </div>
