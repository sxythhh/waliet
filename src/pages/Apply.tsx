...
const STEPS = ["Business Details", "Budget & Goals", "Objectives", "Content Library", "Contact Info"];
export default function Apply() {
  ...
  const uploadLogo = async (): Promise<string | null> => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from("brand-application-logos").upload(filePath, logoFile);
  ...
  const onSubmit = async (values: ApplicationFormValues) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from("brand_applications").insert([{
        name: values.name,
        email: values.email,
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                {currentStep === 0 && <div className="space-y-5">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-col items-center">
                          {logoPreview ? <div className="relative">
                              ...
                              <Button type="button" variant="ghost" size="icon" className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-background border" onClick={removeLogo}>
                                <X className="w-3 h-3" />
                              </Button>
                  ...
                  field
                          ...
                          <FormControl>
                            <Input placeholder="We help businesses scale through content marketing..." {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>} />
                  </div>}
                ...
                {currentStep === 1 && <div className="space-y-6">
                    <FormField control={form.control} name="current_mrr" render={({
                  field
                }) => <FormItem>
                          ...
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="grid gap-3">
                              {["Pre-Revenue", "$0 - $10,000 / month", "$10,000 - $50,000 / month", "$50,000 - $100,000 / month", "$100,000+ / month"].map(option => <label key={option} className={`flex items-center gap-3 p-4 rounded-lg cursor-pointer transition-colors ${field.value === option ? "bg-primary/10 text-primary" : "bg-muted/30 hover:bg-muted/50"}`}>
                    ...
                    <FormField control={form.control} name="monthly_budget" render={({
                  field
                }) => <FormItem>
                          ...
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="grid gap-3">
                              {["$1,000 - $5,000", "$5,000 - $10,000", "$10,000 - $30,000", "$30,000+"].map(option => <label key={option} className={`flex items-center gap-3 p-4 rounded-lg cursor-pointer transition-colors ${field.value === option ? "bg-primary/10 text-primary" : "bg-muted/30 hover:bg-muted/50"}`}>
                                  <RadioGroupItem value={option} className="shrink-0" />
                                  <span className="text-sm font-medium">{option}</span>
                                </label>)}
                            </RadioGroup>
                ...
                {currentStep === 2 && <div className="space-y-6">
                    <FormField control={form.control} name="timeline_commitment" render={({
                  field
                }) => <FormItem>
                          ...
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="grid gap-3">
                              ...
                              ].map(option => <label key={option.value} className={`flex items-center gap-3 p-4 rounded-lg cursor-pointer transition-colors ${field.value === option.value ? "bg-primary/10 text-primary" : "bg-muted/30 hover:bg-muted/50"}`}>
                    ...
                    <FormField control={form.control} name="desired_outcome" render={({
                  field
                }) => <FormItem>
                ...
                {currentStep === 3 && <div className="space-y-5">
                    <FormField control={form.control} name="has_content_library" render={({
                  field
                }) => <FormItem>
                          <FormLabel>
                ...
                <div className="flex justify-between pt-4 mt-2 border-t">
                  <Button type="button" variant="ghost" onClick={prevStep} disabled={currentStep === 0} size="sm">
                    <ArrowLeft className="w-4 h-4 mr-2" />
                    Previous
                  </Button>

                  {currentStep < STEPS.length - 1 ? <Button type="button" onClick={nextStep} size="sm">
                      Next
                      <ArrowRight className="w-4 h-4 ml-1.5" />
                    </Button> : <Button type="submit" disabled={isSubmitting || uploadingLogo} size="sm">
