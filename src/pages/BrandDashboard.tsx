...
import { Pencil, Trash2, TrendingUp, PanelLeft, ArrowRight, Home, LayoutGrid, Menu, BarChart3 } from "lucide-react";
...
export default function BrandDashboard() {
  ...
  const [activeView, setActiveView] = useState<"analytics" | "campaigns" | "home">("analytics");
  const sidebar = useSidebar();
  const isMobile = useIsMobile();
  const handleViewChange = (value: string) => {
    const newView = value as "analytics" | "campaigns" | "home";
  ...
  const showToggle = true; // Always show toggle for analytics
  ...
  return <div className="min-h-screen bg-[#191919]">
        ...
        <>
          <div className="max-w-7xl mx-auto px-4 md:px-8 pt-8">
            <div className="flex items-center justify-between mb-6">
              ...
              {(effectiveView === "campaigns" || effectiveView === "analytics") && <CreateCampaignDialog brandId={brand.id} brandName={brand.name} onSuccess={fetchBrandData} />}
            </div>

            {/* View Toggle */}
            <Tabs value={effectiveView} onValueChange={handleViewChange} className="w-full mb-6">
              <TabsList className="bg-[#202020] border-white/10">
                <TabsTrigger value="analytics" className="gap-2 data-[state=active]:bg-[#2a2a2a]">
      ...
      <>
        ...
        {effectiveView === "home" && hasHomeEmbed && <div className="w-full h-screen">
        ...
        {/* Analytics View */}
        {effectiveView === "analytics" && <div className="max-w-7xl mx-auto px-8 pb-8">
